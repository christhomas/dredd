...
def check_uri(uri):
    if 'github.com/antimatter-studios/dredd/blob/master' in uri:
        path = Path(urlparse(uri).path
                    .replace('antimatter-studios/dredd/blob/master', '.').lstrip('/'))
        return path.exists()
    return True
...
class Tests(unittest.TestCase):
    ...
    def test_relevant_github_repo_but_irrelevant_path(self):
        self.assertTrue(check_uri('https://github.com/antimatter-studios/dredd/issues/820'))

    def test_local_github_project_root_without_slash(self):
        self.assertTrue(check_uri('https://github.com/antimatter-studios/dredd/blob/master'))

    def test_local_github_project_root_with_slash(self):
        self.assertTrue(check_uri('https://github.com/antimatter-studios/dredd/blob/master/'))

    def test_local_github_existing(self):
        self.assertTrue(check_uri('https://github.com/antimatter-studios/dredd/blob/master/README.md'))

    def test_local_github_missing(self):
        self.assertFalse(check_uri('https://github.com/antimatter-studios/dredd/blob/master/foo/bar/doesnt-exist'))
