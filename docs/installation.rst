...
If you are familiar with `Docker <https://docs.docker.com>`__, you can run Dredd in a container without installing Node.js on the host, using any official `Node.js image <https://hub.docker.com/_/node>`__. Specifics of running Dredd inside Docker are:
...
.. tabs::
   ...
   .. group-tab:: macOS, Linux
      ...
      Following line runs the ``dredd`` command inside a Node.js container::

         $ docker run -it -v $PWD:/api -w /api node:20 npx @antimatter-studios/dredd

      As an example of how to pass arguments, following line runs the ``dredd init`` command::

         $ docker run -it -v $PWD:/api -w /api node:20 npx @antimatter-studios/dredd init
   ...
   .. group-tab:: Windows
      ...
      Following line runs the ``dredd`` command inside a Node.js container::
         ...
         C:\Users\Susan> docker run -it -v ${pwd}:/api -w /api node:20 npx @antimatter-studios/dredd

      As an example of how to pass arguments, following line runs the ``dredd init`` command::

         C:\Users\Susan> docker run -it -v ${pwd}:/api -w /api node:20 npx @antimatter-studios/dredd init
...
.. code-block:: text
   ...
   npm install @antimatter-studios/dredd --global
...
- In the root of the project run ``npm install @antimatter-studios/dredd --save-dev``
