...
`Apiary`_ is the main author and maintainer of Dredd’s `upstream repository <https://github.com/christhomas/dredd>`__. Currently responsible people are:
...
To learn the basics of contributing to Dredd, please read the `contributing documentation <https://github.com/christhomas/dredd/blob/master/CONTRIBUTING.md#readme>`__, placed in Dredd's GitHub repository.
...
-   See `existing commits <https://github.com/christhomas/dredd/commits/master>`__ as a reference
...
-   Use ``refactor`` together with ``BREAKING CHANGE:`` for changes in code which only remove features (there doesn't seem to be a better category for that use case) -- see `real-world example <https://github.com/christhomas/dredd/commit/a5fe81b>`__
...
Changelog is in form of `GitHub Releases <https://github.com/christhomas/dredd/releases>`__. Currently it’s automatically generated by `Semantic Release <https://github.com/semantic-release/semantic-release>`__.
...
it uses a public API described in `Apiary Tests API for authenticated test reports <https://github.com/christhomas/dredd/blob/master/packages/dredd/ApiaryReportingApi.apib>`__
...
The documentation is written `as code <http://www.writethedocs.org/guide/docs-as-code/>`__ in the `reStructuredText <http://www.sphinx-doc.org/en/master/usage/restructuredtext/basics.html>`__ format and its source files are located in the `docs <https://github.com/christhomas/dredd/tree/master/docs>`__ directory. It is published automatically by the `ReadTheDocs <https://readthedocs.org/>`__ when the ``master`` branch is updated.
...
GitHub links checker
    Fails the docs build if there's an absolute link (``github.com/christhomas/dredd/blob/master``) to a non-existing local file
...
Dredd is tested on the `AppVeyor <https://www.appveyor.com/>`__, a Windows-based CI. There are still `several known issues <https://github.com/christhomas/dredd/labels/Context%3A%20Windows>`__ when using Dredd on Windows, but the long-term intention is to support it without any compromises.
