 ...
 */
export function removeUnsupportedOptions(config) {
  const result = { ...config };
  delete result.q;
 ...
 */
export function coerceUserOption(config) {
...
export function coerceApiDescriptions(value) {
 ...
 */
export function coerceDeprecatedLevelOption(config) {
...
export function coerceDeprecatedDataOption(config) {
  if (config.data == null) return config;
  const result = { ...config };
  const dataEntries = Object.entries(result.data).map(([location, content]) => {
...
export function coerceColorOption(config) {
...
function coerceOptions(config) {
  ...
  result.method = coerceToArray(result.method).map((m) => m.toUpperCase());
