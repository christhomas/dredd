...
// Builds the messageBodySchema asset for a media type object. Returns undefined when the
// document has no schema to convert, or when a reference in it cannot be resolved - an
// unresolvable reference is the document's problem to report elsewhere, and emitting a
// half-converted schema would fail every response against it.
function generateMessageBodySchema(context, element) {
  const { namespace } = context;
  const mediaTypeObject = element.value;

  if (!mediaTypeObject || typeof mediaTypeObject.get !== 'function') {
    return undefined;
  }

  const schema = mediaTypeObject.get('schema');
  if (!schema) {
    return undefined;
  }
  ...
  const asset = new namespace.elements.Asset(JSON.stringify(jsonSchema));
...
function parseMediaTypeObject(context, MessageBodyClass, element) {
  ...
  const parseMediaType = pipeParseResult(namespace,
    ...
    (mediaTypeObject) => {
      ...
      // A JSON Schema for the body, which is what lets a consumer validate a response
      // against what the document actually says: required properties must be present,
      // optional ones may be absent, and an optional one that IS present is still checked.
      // Without it a consumer can only compare against the generated example above, in which
      // every key looks required.
      //
      // Converted from the unparsed schema rather than from the dataStructure above, because
      // the reference the document wrote is still intact there.
      // Deliberately not gated on the parsed dataStructure above. The schema asset is built
      // from the unparsed schema, so a document whose schema the dataStructure parse could not
      // represent still gets a schema a consumer can validate against.
      if (context.options.generateMessageBodySchema && isJSONMediaType(mediaType)) {
        const asset = generateMessageBodySchema(context, element);
        if (asset) {
          message.push(asset);
        }
      }
